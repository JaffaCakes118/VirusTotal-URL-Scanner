...
const rateLimitManager = {
    ...
    pauseAndSetRetryAlarm: function() {
        this.isPaused = true;
        browser.alarms.create('rateLimitRetryAlarm', { delayInMinutes: 1 });
    },
    resume: function() {
...
async function initializeExtension(details) {
    ...
    if (details && details.reason === 'install') {
        await browser.storage.local.set({ removedDefaultDomains: [] });
    }
    ...
    if (needsUpdate) {
        await browser.storage.local.set({ whitelistedDomains: updatedWhitelist });
    }

    await browser.alarms.create('mainQueueAlarm', {
        delayInMinutes: 1,
        periodInMinutes: 1
    });
...
async function completeItem(item, scanQueue, result, error) {
    ...
    const { completedScans = [] } = await browser.storage.local.get('completedScans');
    completedScans.unshift({ url: item.url, result, error: error ? error.message : null });
    if (completedScans.length > 500) completedScans.pop();

    await browser.storage.local.set({ scanQueue, completedScans });
...
async function addToQueue(url) {
    ...
    try {
        ...
        const { whitelistedDomains = [], removedDefaultDomains = [] } = await browser.storage.local.get(['whitelistedDomains', 'removedDefaultDomains']);
    ...
    const { scanQueue = [], completedScans = [] } = await browser.storage.local.get(['scanQueue', 'completedScans']);
    ...
    await browser.storage.local.set({ scanQueue });
    await processQueue();
}
...
async function processQueue() {
    ...
    try {
        const { apiKey, consentGiven } = await browser.storage.local.get(['apiKey', 'consentGiven']);
        ...
        const { scanQueue = [] } = await browser.storage.local.get('scanQueue');
...
browser.runtime.onMessage.addListener((request, sender, sendResponse) => {
    if (request.type === 'API_KEY_UPDATED') {
        ...
        (async () => {
            ...
            let { scanQueue = [] } = await browser.storage.local.get('scanQueue');
        ...
        (async () => {
            ...
            await browser.storage.local.set({ completedScans: [] });
...
browser.runtime.onStartup.addListener(() => initializeExtension({ reason: 'startup' }));
...
async function verifyAndSaveApiKey(apiKey) {
    try {
        ...
        const data = await response.json();
        await browser.storage.local.set({ apiKey, apiQuota: data.data.attributes.quotas, dailyQuotaExceeded: false });
        return { success: true };
    } catch (error) {
        await browser.storage.local.remove(['apiKey', 'apiQuota', 'dailyQuotaExceeded']);
