...
async function updateUI() {
    ...
    chrome.storage.local.get(['scanQueue', 'completedScans', 'apiKey', 'whitelistedDomains', 'removedDefaultDomains', 'apiQuota', 'dailyQuotaExceeded'], (data) => {
...
addWhitelistBtn.addEventListener('click', () => {
    ...
    if (REMOVABLE_DEFAULT_DOMAINS.includes(newDomain)) {
        chrome.storage.local.get('removedDefaultDomains', ({ removedDefaultDomains = [] }) => {
            const updatedRemovedList = removedDefaultDomains.filter(d => d !== newDomain);
            chrome.storage.local.set({ removedDefaultDomains: updatedRemovedList });
        });
    } else {
        ...
        chrome.storage.local.get('whitelistedDomains', ({ whitelistedDomains = [] }) => {
            if (!whitelistedDomains.includes(newDomain)) {
                ...
                chrome.storage.local.set({ whitelistedDomains: updatedList });
            }
        });
    }
    whitelistInput.value = '';
...
saveKeyButton.addEventListener('click', () => {
    ...
    if (apiKey) {
        ...
        chrome.runtime.sendMessage({ type: 'API_KEY_UPDATED', apiKey: apiKey }, (response) => {
...
document.addEventListener('DOMContentLoaded', () => {
        ...
        if(popoutBtn) {
            popoutBtn.addEventListener('click', () => {
                chrome.windows.create({
                    url: chrome.runtime.getURL('popup.html?isPopup=false'),
                    type: 'popup',
                    width: POPOUT_WIDTH,
    ...
    chrome.storage.local.get(['theme', 'consentGiven'], (data) => {
...
queueList.addEventListener('click', (e) => {
    ...
    if (removeButton) {
        ...
        if (urlToRemove) {
            chrome.runtime.sendMessage({ type: 'REMOVE_FROM_QUEUE', url: urlToRemove });
        }
    }
});